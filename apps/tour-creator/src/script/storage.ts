...
// Shared replacer function
export const tourReplacer = (key: string, value: any) => {
    ...
    for (const project of arr) {
        const response = await fetch("/save-tour", {
            ...
            body: JSON.stringify({
                ...
                tourData: JSON.parse(JSON.stringify(project, tourReplacer))
            })
        });
...
export const isEditedSincePublish = async (tourId: string): Promise<boolean> => {
    const token = getAuthToken();
    if (!token) throw new Error("Not authenticated");

    const response = await fetch(`/is-edited-since-publish?tourId=${tourId}`, {
        headers: { "Authorization": `Bearer ${token}` }
    });
...
export const getSavedTourJson = async (tourId: string): Promise<string | null> => {
    const token = getAuthToken();
    ...
    const match = data.tours.find((t: any) => t.tourId.toString() === tourId);
    if (!match || !match.tourData) return null;

    return JSON.stringify(match.tourData.tour);
};
